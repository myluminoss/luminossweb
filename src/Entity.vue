    ...
    <div class="content">
        ...
        <div class="start" v-if="false">
            ...
            <div class="tips">
                ...
                " Hello! Ready to unlock the power of AI?
            ...
            <div class="chatList">
                ...
                <div class="history">
                    ...
                    <div class="chatInput">
                        ...
                        <div class="_input">
                            <input class="inputEx" @input="handlerInput" placeholder="What's bothering you ?"
...
const tips2 = ref(`Together, we can turn your imagination into reality! ${`"`}`)
...
const tips33 = ref(`Together, we can turn your imagination into reality! ${`"`}`)
...
const initTipsFunc22 = () => {
    isEmpty.value = true;
    let currentTipIndex = 0; // 用于记录当前正在执行打字机效果的提示语索引
    let interval: any = null; // 存储定时器的引用
    const executeTypingEffect = (text: string) => {
        let i = 0;
        const timerFunc = () => {
            i++;
            const str = text.substr(0, i).split("\n").map(item => `<br/>${item}`).join("");
            if (i > text.length) {
                ...
                if (interval) {
                    ...
                    if (currentTipIndex === 0) {
                        // 如果当前是第一个提示语执行完了，开始执行第二个提示语的打字机效果
                        currentTipIndex++;
                        startTypingEffect();
...
const sendTest = async () => {
    ...
    loading.value = true;
    isRight.value = true;
    let res: any = await axios.post('https://ai.luminoss.ai/chat', {
