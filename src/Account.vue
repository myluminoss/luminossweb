        ...
        <div class="asset_cards__DvwL2">
          <div class="asset_card__QFGy1 asset_card2__Y0AxO">
            <div class="asset_cLeft__5SCPu">
              <div class="asset_lTitle__zReuz">
                ...
                <div
                  class="asset_buttons__Xn8QN asset_pcButtons__1EFYv"
                  v-show="false"
                >
                  <div
                    style="opacity: 0"
                    class="asset_button__vitgO asset_deposit__8ep8l"
                  >
              ...
              <div class="asset_rNodes__hC4GU">
                <strong>{{ myInfoObj?.earn || 0 }}</strong>
              </div>
            </div>
            <div class="asset_mobilePurchaseButton__SjUVY" v-show="false">
              <div
                class="Button_button__fo9Wi Button_default__ulIdO Button_large__lOuOH"
                style="
                  width: 100%;
                  height: 36px;
                  border-radius: 10px;
          ...
          <div class="asset_card__QFGy1 asset_card1__oE3Kk">
            <div class="asset_cLeft__5SCPu">
              ...
              <div
                class="asset_buttons__Xn8QN asset_mobileButtons__Tvacf"
                v-show="false"
              >
                <div class="asset_button__vitgO asset_deposit__8ep8l">
                  <img src="@/assets/img/icon-deposit-w.svg" alt="" /><span
                    >Deposit</span
                  >
                </div>
                <div class="asset_button__vitgO asset_widthdraw__sRog3">
                  <img src="@/assets/img/icon-withdraw-w.svg" alt="" /><span
                    >Withdraw</span
                  >
                </div>
              </div>
          ...
          <div class="stab_tabsWrap__pHwOO">
            <div class="stab_tabsContent__X1Lhe">
              <div
                ...
                :class="index == activeIndex ? 'stab_active__hccBq' : ''"
                :key="index"
                class="stab_item__H4RdD"
              >
                <span>{{ item.name }}</span>
              </div>
            </div>
                  ...
                  <div class="account_folowsContent__YT5IZ">
                    <table
                      v-show="
                        orderDatas.length > 0 ||
                        pointDatas.length > 0 ||
                        rewardDatas.length > 0
                      "
                      cellpadding="10"
                      cellspacing="0"
                      class="account_fTable__YgCX7"
                    >
                      <thead>
                        <tr>
                          <td
...
const pointList = async () => {
  ...
  pointDatas.value = [];
  let listRes: any = await getPointList({
    page: page.value,
    size: pageSize.value,
  }).catch((err) => {
    console.error(err);
...
const rewardList = async () => {
  ...
  rewardDatas.value = [];
  let listRes: any = await getRewardList({
    page: page.value,
    size: pageSize.value,
  }).catch((err) => {
    console.error(err);
...
const orderList = async () => {
  ...
  orderDatas.value = [];
  let listRes: any = await getOrderList({
    page: page.value,
    size: pageSize.value,
  }).catch((err) => {
    console.error(err);
