                ...
                <div class="history">
                    <div class="history-list " :class="closeStatus" ref="scrollBox">
                        <div class="history-list2">
                            <div class="history-item " :class="item.class" v-for="(item, index) in msgList"
                                :key="index">
                                <div class="item-header">
                                    <img v-if="item.class == 'other'" src="@/assets/entiry/UI_Bg_Head_02.png" />
                                ...
                                <div class="item-content">
                                    ...
                                    <div class="contentEx selectAI" v-if="item.isEx">
                                        <div class="item" v-for="(item, index) in aiEntityList"
                                            @click="onClickSelectAI(item)" :key="index">
                                            {{ index + 1 }}.{{ item.title }}
                                    ...
                                    <!--    -->
                                    <div class="contentEx imgUpload" v-if="item?.showUpdate" @click="onClickSelectImg">
                                        <div class="item upload">
                                            <img class="icon2" v-if="fileImgShow != ''" :src="fileImgShow" />
                                            <img class="icon" v-else src="@/assets/entiry/upload-img.svg" />

                                        </div>
                                        <input @change="imgChange" ref="fileEle" class="fileEle" type="file"
                                            accept="image/*" />

                                    </div>
                                    <!--   -->
                                    <div class="contentEx confirmBtn" v-if="item?.showConfirmBtn"
                    ...
                    <div class="chatInput">
                        ...
                        <div class="_input">
                            <input ref="inputEx" class="inputEx" @input="handlerInput"
                                placeholder="What's bothering you ?" v-model="inputValue" @keyup.enter="handleEnter" />
                        </div>
                        <div class="iconBox" @click="handleEnter">
                            <img v-if="!loading" class="icon" src="@/assets/entiry/UI_Btn_Icon_13.png" />
                            <img v-else class="loading" src="@/assets/entiry/ui_loading.png" />
                        </div>
                    </div>
...
const onClickConfirm = async () => {
    ...
    axios.post('https://srv.luminoss.ai/aiAgent/createAgent', sendData).then(res => {
...
import imgLoad from "@/assets/entiry/upload-img.svg"
...
const addTxt = (num, content) => {
    ...
    const timerFunc = () => {
        i++;
        const str = content.substr(0, i).split("\n").map(item => `<br/>${item}`).join("");
        if (i > content.length) {
...
.agent-content {
    ...
    .chat-content {
        ...
        .roleSize {
            ...
            &:hover {
                filter: drop-shadow(0px 0 30px var(--c)) drop-shadow(0px 0 10px var(--c));
